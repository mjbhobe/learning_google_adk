...
### An easy way to setup this structure
* Navigate to `parent_folder` in a command shell and run the following command:
```bash
$>  adk create greeting_agent
```
* Replace greeting_agent with name of agent you want to create
* This will ask you for some inputs, such as using Google LLM or not, using Vertex AI or AI Studio and LLM API Key
* Then it will create the following folder structure

```
parent_folder/
    greeting_agent/         # This name will be same as agent name above
        __init__.py       # will be generated with line 'from . import agent'
        agent.py          # Shell code for agent
        .env              # file with API key you specify on command line
```
